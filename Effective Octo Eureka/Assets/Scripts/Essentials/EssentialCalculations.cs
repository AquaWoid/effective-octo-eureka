...
namespace EurekaEssentials
{
    public class EssentialCalculations
    {


        public GameObject closest_Object(Transform transform, string objectName)
        {
            GameObject[] gameObjects;
            gameObjects = GameObject.FindGameObjectsWithTag(objectName);
        ...
        public int OrientationRotation(Transform entity, Transform entityFacing)
        {
            //0 = facing from left
            //1 = facing from down
            //2 = facing from right
            ...
            Vector2 v = entity.position - entityFacing.position;
            float a = Mathf.Atan2(v.y, v.x) * Mathf.Rad2Deg;
            float OrientationRaw = a;
            int OrientationRotation = (int)((Mathf.Round(a / 90f) + 4) % 4);
